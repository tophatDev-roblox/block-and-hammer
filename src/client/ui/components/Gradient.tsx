...
const Gradient: React.FC<GradientProps> = ({ styles: { colors, transparency, rotation } }) => {
	...
	useEffect(() => {
		...
		if (keypoints.size() >= 2 && keypoints[0].Time === 0 && keypoints[keypoints.size() - 1].Time === 1) {
			...
			warn('[client::ui/Gradient] color must have at least 2 keypoints, start at 0 and end at 1');
	...
	useEffect(() => {
		...
		if (keypoints.size() >= 2 && keypoints[0].Time === 0 && keypoints[keypoints.size() - 1].Time === 1) {
			...
			warn('[client::ui/Gradient] transparency must have at least 2 keypoints, start at 0 and end at 1');
